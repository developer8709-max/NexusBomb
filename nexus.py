...
def display_banner():
    ...
    print(f"{Fore.CYAN}DEVELOPER EXPLOITS POWER FULL BOMBER {Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Made By : DEVELOPER_EXPLOITS{Style.RESET_ALL}")

def stop_script():
    global running
    running = False
    print(f"{Fore.YELLOW}[INFO] Stopping DEVELOPER X BOMBER SEAMLESS ULTRA FAST...{Style.RESET_ALL}")
...
async def main():
    ...
    if not phone.isdigit() or len(phone) != 10:
        ...
        return
    print(f"{Fore.YELLOW}[INFO] Starting DEVELOPER X BOMBER SEAMLESS ULTRA FAST for {phone}... Press Ctrl+C to stop.{Style.RESET_ALL}")
    try:
